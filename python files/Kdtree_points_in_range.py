...
class KdTree:
    ...
    def points_in_range(self, query_rectangle, memo= None):
        ...
        if self.is_leaf:
            ...
            for i in self.points:
                if i.in_box(query_rectangle[0], query_rectangle[1]):
                    matches.append(i)
        else:
            matches = []
            if self.leftorbottom:
                matches += self.leftorbottom.points_in_range(query_rectangle, memo)
                memo[self] = matches
            if self.rightortop:
                matches += self.rightortop.points_in_range(query_rectangle, memo)
                memo[self] = matches
        return matches
